...
data_dir = 'C:\Users\39320\Desktop\EPFL\Thesis code\tSCS_Continuos\NEW_PROTOCOL\MATRIX\EMG\LSWING'; 
...
% Delete the last column
 max_values_LswingPROX = sumMatrixTOT(:, [1:7,15]);
 max_values_LswingDIST = sumMatrixTOT(:, 8:15);
...
for i = 1:7
    ...
    smoothed_values_stance = sgolayfilt(max_values_Lswing_sortedDIST(i, :), sgolay_order, sgolay_frame);
...
for i = 1:7
    ...
    % Apply Savitzky-Golay filter
    smoothed_values_stance = sgolayfilt(max_values_Lswing_sortedPROX(i, :), sgolay_order, sgolay_frame);
    plot(sorted_frequencies,smoothed_values_stance, 'LineWidth', 2); % Plot logaritmico
    hold on;
    %xline(100, '--r', 'LineWidth', 1.5); % Linea verticale tratteggiata rossa a 100 Hz
